...
class NewCardService @Inject constructor(private val dataStore: DataStore<Preferences>) {
        ...
        return withContext(Dispatchers.IO) {
            ...
            return@withContext dataStore.data.map { preferences ->
                User(
                    ...
                    cards = gson.fromJson<ArrayList<Card>?>(
                        preferences[stringPreferencesKey("cards")],
    ...
    suspend fun addCard(newCards: List<Card>): Preferences {
        return withContext(Dispatchers.IO) {
            val gson = Gson();
            val cardsInJson = gson.toJson(newCards);
            dataStore.edit { preferences ->
                preferences[stringPreferencesKey("cards")] = cardsInJson;
            }
        }
    }
}